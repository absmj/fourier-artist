const rect = attrs => {
  ...
  const ellipse = attrs => {
    console.log(attrs)
    const cx = +attrs.cx.value
    const cy = +attrs.cy.value
    const rx = attrs.rx?.value ? +attrs.rx.value : +attrs.r.value
    const ry = attrs.ry?.value ? +attrs.ry.value : +attrs.r.value
    return [
      `M${cx + rx} ${cy}`,
